...
% Filter 2: only times pass where ALL (RR0, dBZ, all predictors) are non-NAN
% This assures the same data-base for all predictor tests

    indx_del = [];

    % loop over all timesteps that made it through filter 1
    for t = 1 : length(all_RR0_dBZ_predictors)
       dummy = find(isnan(all_RR0_dBZ_predictors(t,:))); % in a row, find all NaN's
       if ~isempty(dummy) % if there was at least one NaN in the row ...
           indx_del(length(indx_del)+1) = t; % ... mark this row for deletion
       end
    end

    % delete rows from all arrays
    all_RR0_dBZ_predictors(indx_del,:) = [];
    all_DateTime_UTC(indx_del) = [];
